/**
 * 2025.9.29
 * 8.1.2三角形光栅化算法 incrementalalgorithm版,暂未实现
...
float ComputeAlpha(float xs,float ys, float x,float y);
float ComputeBeta(float xs,float ys, float x,float y);
float ComputeGamma(float xs,float ys, float x,float y);
...
//三角形ABC面积
static float sabc = (y1 - y2) * x0 + (x2 - x1) * y0 + x1 * y2 - x2 * y1;
static float spab;
static float spbc;
static float spac;
//颜色
...
int main(int argc, char const *argv[])
{
    ...
    for(float y=ymin; y<=ymax; y++)
    {
        ...
        {
            float alpha = ComputeAlpha(xmin,ymin,x,y);
            float beta = ComputeBeta(xmin,ymin,x,y);
            float gamma = ComputeGamma(xmin,ymin,x,y);
...
//计算重心坐标alpha
//采用增量型，新增一个初始值xs,ys，只用函数计算一次，后续的使用增量计算
float ComputeAlpha(float xs,float ys, float x,float y)
{   
    //定义增量
    float A = y1 - y2;
